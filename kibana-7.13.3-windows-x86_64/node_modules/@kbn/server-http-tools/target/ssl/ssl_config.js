...
const crypto_1 = require("@kbn/crypto");
const crypto_2 = require("crypto");
const fs_1 = require("fs");
const protocolMap = new Map([
    ['TLSv1', crypto_2.constants.SSL_OP_NO_TLSv1],
    ['TLSv1.1', crypto_2.constants.SSL_OP_NO_TLSv1_1],
    ['TLSv1.2', crypto_2.constants.SSL_OP_NO_TLSv1_2],
    // @ts-expect-error According to the docs SSL_OP_NO_TLSv1_3 should exist (https://nodejs.org/docs/latest-v12.x/api/crypto.html)
    ['TLSv1.3', crypto_2.constants.SSL_OP_NO_TLSv1_3],
]);
exports.sslSchema = config_schema_1.schema.object({
    ...
    cipherSuites: config_schema_1.schema.arrayOf(config_schema_1.schema.string(), {
        defaultValue: crypto_2.constants.defaultCoreCipherList.split(':'),
    }),
    ...
    supportedProtocols: config_schema_1.schema.arrayOf(config_schema_1.schema.oneOf([
        config_schema_1.schema.literal('TLSv1'),
        config_schema_1.schema.literal('TLSv1.1'),
        config_schema_1.schema.literal('TLSv1.2'),
        config_schema_1.schema.literal('TLSv1.3'),
...
class SslConfig {
    ...
    constructor(config) {
        ...
        if (config.keystore?.path) {
            const { key, cert, ca } = crypto_1.readPkcs12Keystore(config.keystore.path, config.keystore.password);
            if (!key) {
        ...
        if (config.truststore?.path) {
            const ca = crypto_1.readPkcs12Truststore(config.truststore.path, config.truststore.password);
            addCAs(ca);
        }
...
const readFile = (file) => fs_1.readFileSync(file, 'utf8');
