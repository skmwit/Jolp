...
function registerFieldHistogramsRoutes({
  ...
  router.post({
    ...
    try {
      const resp = await (0, _shared_imports.getHistogramsForFields)(ctx.core.elasticsearch.client, indexPatternTitle, query, fields, samplerShardSize, // @ts-expect-error script is not compatible with StoredScript from @elastic/elasticsearch: string is not supported
      runtimeMappings);
      return res.ok({
        body: resp
      });
