...
function wrapEsError(err, statusCodeToMessageMap = {}) {
  ...
  } = body; // If no custom message if specified for the error's status code, just
  ...
  if (!statusCodeToMessageMap[statusCode]) {
    ...
    boomError.output.payload.cause = causedByChain.length ? causedByChain : defaultCause; // Set error message based on the root cause
