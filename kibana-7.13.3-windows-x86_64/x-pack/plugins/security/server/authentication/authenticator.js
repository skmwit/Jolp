...
class Authenticator {
  ...
  async logout(request) {
    ...
    if (suggestedProviderName) {
      await this.invalidateSessionValue(request); // Provider name may be passed in a query param and sourced from the browser's local storage;
  ...
  *providerIterator(suggestedProviderName) {
  ...
  async invalidateSessionValue(request) {
