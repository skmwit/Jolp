...
exports.SAMLAuthenticationProvider = exports.SAMLLogin = void 0;
...
class SAMLAuthenticationProvider extends _base.BaseAuthenticationProvider {
  ...
  constructor(options, samlOptions) {
    ...
    _defineProperty(this, "useRelayStateDeepLink", void 0);
  ...
  async login(request, attempt, state) {
    ...
    const authenticationResult = state ? await this.authenticateViaState(request, state) : _authentication_result.AuthenticationResult.notHandled(); // Let's check if user is redirected to Kibana from IdP with valid SAMLResponse.
    ...
    if (authenticationResult.redirected()) {
      ...
      this.logger.debug(`Failed to perform a login: ${authenticationResult.error && authenticationResult.error.message}`);
    }
  ...
  async authenticate(request, state) {
    ...
    return authenticationResult.notHandled() && canStartNewSession(request) ? this.captureRedirectURL(request) : authenticationResult;
  }
  /**
  ...
  async logout(request, state) {
    this.logger.debug(`Trying to log user out via ${request.url.pathname}${request.url.search}.`); // Normally when there is no active session in Kibana, `logout` method shouldn't do anything
    ...
    if (state !== null && state !== void 0 && state.accessToken || isIdPInitiatedSLORequest || isSPInitiatedSLOResponse) {
      try {
        ...
        const redirect = isIdPInitiatedSLORequest ? await this.performIdPInitiatedSingleLogout(request, this.realm || (state === null || state === void 0 ? void 0 : state.realm)) : state ? await this.performUserInitiatedSingleLogout(state === null || state === void 0 ? void 0 : state.accessToken, state === null || state === void 0 ? void 0 : state.refreshToken) : // Once Elasticsearch can consume logout response we'll be sending it here. See https://github.com/elastic/elasticsearch/issues/40901
        null; // Having non-null `redirect` field within logout response means that IdP
  ...
  async loginWithSAMLResponse(request, samlResponse, relayState, state) {
    this.logger.debug('Trying to log in with SAML response payload.'); // If we have a `SAMLResponse` and state, but state doesn't contain all the necessary information,
    ...
    try {
      ...
      this.logger.debug(`Failed to log in with SAML response: ${err.message}`); // Since we don't know upfront what realm is targeted by the Identity Provider initiated login
  ...
  async loginWithNewSAMLResponse(request, samlResponse, relayState, existingState) {
    this.logger.debug('Trying to log in with SAML response payload and existing valid session.'); // First let's try to authenticate via SAML Response payload.
  ...
  async authenticateViaHandshake(request, redirectURL) {
    ...
    try {
      ...
      this.logger.debug('Redirecting to Identity Provider with SAML request.'); // Store request id in the state so that we can reuse it once we receive `SAMLResponse`.
  ...
  async performUserInitiatedSingleLogout(accessToken, refreshToken) {
    this.logger.debug('Single logout has been initiated by the user.'); // This operation should be performed on behalf of the user with a privilege that normal
  ...
  async performIdPInitiatedSingleLogout(request, realm) {
    this.logger.debug('Single logout has been initiated by the Identity Provider.'); // Prefer realm name if it's specified, otherwise fallback to ACS.
  ...
  captureRedirectURL(request) {
    const searchParams = new URLSearchParams([[_constants.NEXT_URL_QUERY_STRING_PARAMETER, `${this.options.basePath.get(request)}${request.url.pathname}${request.url.search}`], ['providerType', this.type], ['providerName', this.options.name]]);
    return _authentication_result.AuthenticationResult.redirectTo(`${this.options.basePath.serverBasePath}/internal/security/capture-url?${searchParams.toString()}`, // Here we indicate that current session, if any, should be invalidated. It is a no-op for the
...
_defineProperty(SAMLAuthenticationProvider, "type", 'saml');