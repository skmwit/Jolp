...
class TokenAuthenticationProvider extends _base.BaseAuthenticationProvider {
  ...
  async login(request, {
    ...
    try {
      ...
      return _authentication_result.AuthenticationResult.succeeded(this.authenticationInfoToAuthenticatedUser( // @ts-expect-error @elastic/elasticsearch GetUserAccessTokenResponse declares authentication: string, but expected AuthenticatedUser
      authenticationInfo), {
        authHeaders: {
