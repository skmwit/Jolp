...
var _oidc = require("../../authentication/providers/oidc");
...
function defineOIDCRoutes({
  ...
  for (const path of ['/api/security/oidc/callback', '/api/security/v1/oidc']) {
    router.get({
      ...
      const serverBasePath = basePath.serverBasePath; // An HTTP GET request with a query parameter named `authenticationResponseURI` that includes URL fragment OpenID
  ...
  async function performOIDCLogin(request, response, loginAttempt) {
    try {
      ...
      const authenticationResult = await getAuthenticationService().login(request, {
        provider: {
          type: _oidc.OIDCAuthenticationProvider.type
        },
        value: loginAttempt
      });
