  ...
  router.get({
    ...
    try {
      ...
      if (elasticsearchRole) {
        return response.ok({
          body: (0, _model.transformElasticsearchRoleToRole)( // @ts-expect-error @elastic/elasticsearch `XPackRole` type doesn't define `applications` and `transient_metadata`.
          elasticsearchRole, request.params.name, authz.applicationName)
        });
      }
