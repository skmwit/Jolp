...
function defineGetAllRolesRoutes({
  ...
  router.get({
    ...
    try {
      ...
      return response.ok({
        body: Object.entries(elasticsearchRoles).map(([roleName, elasticsearchRole]) => (0, _model.transformElasticsearchRoleToRole)( // @ts-expect-error @elastic/elasticsearch `XPackRole` type doesn't define `applications` and `transient_metadata`.
        elasticsearchRole, roleName, authz.applicationName)).sort((roleA, roleB) => {
