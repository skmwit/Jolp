...
async function getTotalCount(esClient, kibanaIndex) {
  ...
  return {
    countTotal: Object.keys(aggs).reduce((total, key) => parseInt(aggs[key], 0) + total, 0),
    countByType: Object.keys(aggs).reduce( // ES DSL aggregations are returned as `any` by esClient.search
...
async function getInUseTotalCount(esClient, actionsBulkGet, kibanaIndex) {
  const scriptedMetric = {
    scripted_metric: {
  ...
  const actions = await actionsBulkGet(bulkFilter); // filter out preconfigured connectors, which are not saved objects and return
  // an error in the bulk response

  const actionsWithActionTypeId = actions.saved_objects.filter(action => {
    var _action$attributes;

    return (action === null || action === void 0 ? void 0 : (_action$attributes = action.attributes) === null || _action$attributes === void 0 ? void 0 : _action$attributes.actionTypeId) != null;
  });
  const countByActionTypeId = actionsWithActionTypeId.reduce((actionTypeCount, action) => {
  ...
  const preconfiguredAlertHistoryConnector = actions.saved_objects.filter(action => action.id === _common.AlertHistoryEsIndexConnectorId);
  return {
