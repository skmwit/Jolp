  ...
  return ({
    ...
    return {
      async run() {
        ...
        return Promise.all([(0, _actions_telemetry.getTotalCount)(esClient, kibanaIndex), (0, _actions_telemetry.getInUseTotalCount)(esClient, actionsBulkGet, kibanaIndex)]).then(([totalAggegations, totalInUse]) => {
          return {
            state: {
              runs: (state.runs || 0) + 1,
