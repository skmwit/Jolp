...
const duplicateSelectedRules = () => {
  cy.get(_alerts_detection_rules.BULK_ACTIONS_BTN).click({
    force: true
  });
  cy.get(_alerts_detection_rules.DUPLICATE_RULE_BULK_BTN).click();
};
/**
...
exports.duplicateSelectedRules = duplicateSelectedRules;
...
const duplicateRuleFromMenu = () => {
  ...
  cy.get(_alerts_detection_rules.DUPLICATE_RULE_MENU_PANEL_BTN).pipe($el => $el.trigger('click'));
};
/**
...
const checkDuplicatedRule = () => {
  cy.contains(_alerts_detection_rules.RULE_NAME, _rule.duplicatedRuleName).parents(_alerts_detection_rules.RULES_ROW).find(_alerts_detection_rules.RULE_SWITCH).should('have.attr', 'aria-checked', 'false');
};

exports.checkDuplicatedRule = checkDuplicatedRule;
...
const changeRowsPerPageTo = rowsCount => {
  ...
  cy.get((0, _alerts_detection_rules.rowsPerPageSelector)(rowsCount)).pipe($el => $el.trigger('click')).should('not.be.visible');
  waitForRulesTableToBeRefreshed();
};
