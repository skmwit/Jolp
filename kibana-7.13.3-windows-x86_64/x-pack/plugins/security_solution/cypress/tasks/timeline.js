...
const goToNotesTab = () => {
  cy.root().pipe($el => {
    $el.find(_timeline.NOTES_TAB_BUTTON).trigger('click');
    return $el.find(_timeline.NOTES_TEXT_AREA);
  }).should('be.visible');
  return cy.root().find(_timeline.NOTES_TAB_BUTTON);
};
...
const goToQueryTab = () => {
  cy.root().pipe($el => {
    $el.find(_timeline.QUERY_TAB_BUTTON).trigger('click');
    return $el.find(_timeline.QUERY_TAB_BUTTON);
...
const addNotesToTimeline = notes => {
  goToNotesTab().then(() => {
    ...
    cy.root().pipe($el => {
      $el.find(_timeline.ADD_NOTE_BUTTON).trigger('click');
      return $el.find(_timeline.NOTES_TAB_BUTTON).find('.euiBadge');
    }).should('have.text', '1');
  });
  goToQueryTab();
  goToNotesTab();
...
const addDataProvider = filter => {
  ...
  cy.get(_timeline.TIMELINE_DATA_PROVIDER_VALUE).should('have.focus'); // make sure the focus is ready before start typing
...
const closeTimeline = () => {
  cy.root().pipe($el => {
    $el.find(_timeline.CLOSE_TIMELINE_BTN).filter(':visible').trigger('click');
    return $el.find(_timeline.QUERY_TAB_BUTTON);
...
const createNewTimeline = () => {
  cy.get(_timeline.TIMELINE_SETTINGS_ICON).filter(':visible').pipe($el => $el.trigger('click')).should('be.visible');
  cy.wait(300);
  cy.get(_timeline.CREATE_NEW_TIMELINE).eq(0).pipe($el => $el.trigger('click'));
};
...
const openTimelineById = timelineId => {
  ...
  cy.root().pipe($el => {
    $el.find((0, _timeline.TIMELINE_TITLE_BY_ID)(timelineId)).trigger('click');
    return $el.find(_timeline.QUERY_TAB_BUTTON).find('.euiBadge');
  }).should('be.visible');
  return cy.root().find((0, _timeline.TIMELINE_TITLE_BY_ID)(timelineId));
};
