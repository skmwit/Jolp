...
const closeFirstAlert = () => {
  cy.get(_alerts.TIMELINE_CONTEXT_MENU_BTN).first().pipe($el => $el.trigger('click')).should('be.visible');
  cy.get(_alerts.CLOSE_ALERT_BTN).pipe($el => $el.trigger('click')).should('not.be.visible');
};

exports.closeFirstAlert = closeFirstAlert;

const closeAlerts = () => {
  cy.get(_alerts.TAKE_ACTION_POPOVER_BTN).first().pipe($el => $el.trigger('click')).should('be.visible');
  cy.get(_alerts.CLOSE_SELECTED_ALERTS_BTN).pipe($el => $el.trigger('click')).should('not.be.visible');
};
