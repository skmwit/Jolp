...
const createAndActivateRule = () => {
  ...
  cy.get(_create_new_rule.BACK_TO_ALL_RULES_LINK).click({
    force: true
  });
  cy.get(_create_new_rule.BACK_TO_ALL_RULES_LINK).should('not.exist');
};
...
const fillDefineMachineLearningRuleAndContinue = rule => {
  rule.machineLearningJobs.forEach(machineLearningJob => {
    cy.get(_create_new_rule.MACHINE_LEARNING_DROPDOWN_INPUT).click({
      force: true
    });
    cy.get(_create_new_rule.MACHINE_LEARNING_DROPDOWN_INPUT).type(`${machineLearningJob}{enter}`);
    cy.get(_create_new_rule.MACHINE_LEARNING_DROPDOWN_INPUT).type('{esc}');
  });
  ...
  cy.get(_create_new_rule.MACHINE_LEARNING_DROPDOWN_INPUT).should('not.exist');
};
...
const waitForAlertsToPopulate = async (alertCountThreshold = 1) => {
  cy.waitUntil(() => {
