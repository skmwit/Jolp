...
describe('Exceptions modal', () => {
  ...
  it('Does not overwrite values of nested entry items', () => {
    ...
    cy.get(_exceptions2.ADD_OR_BTN).click();
    (0, _exceptions.addExceptionEntryFieldValueOfItemX)('agent.name', 1, 0);
    ...
    cy.get(_exceptions2.ADD_NESTED_BTN).scrollIntoView();
    cy.get(_exceptions2.ADD_NESTED_BTN).focus().click();
    ...
    cy.get(_exceptions2.EXCEPTION_ITEM_CONTAINER).eq(1).find(_exceptions2.FIELD_INPUT).eq(0).should('have.text', 'agent.name');
