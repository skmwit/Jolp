...
var _alerts_details2 = require("../../tasks/alerts_details");
...
describe('indicator match', () => {
  describe('Detection rules, Indicator Match', () => {
    ...
    before(() => {
      ...
      (0, _es_archiver.esArchiverLoad)('suspicious_source_event');
    });
    after(() => {
      (0, _es_archiver.esArchiverUnload)('threat_indicator');
      (0, _es_archiver.esArchiverUnload)('suspicious_source_event');
    });
      ...
      describe('Indicator mapping', () => {
        ...
        it('Does NOT show invalidation text when there is a valid "index field" and a valid "indicator index field"', () => {
          (0, _create_new_rule.fillIndicatorMatchRow)({
            indexField: _rule.newThreatIndicatorRule.indicatorMappingField,
        ...
        it('Deletes the second row when you have three rows. The first row is valid data, the second row is invalid data, and the third row is valid data. Third row should shift up correctly', () => {
          ...
          (0, _create_new_rule.getIndicatorIndexComboField)(1).should('text', _rule.newThreatIndicatorRule.indicatorMappingField);
          (0, _create_new_rule.getIndicatorMappingComboField)(1).should('text', _rule.newThreatIndicatorRule.indicatorIndexField);
          (0, _create_new_rule.getIndicatorIndexComboField)(2).should('text', _rule.newThreatIndicatorRule.indicatorMappingField);
        ...
        it('Can add two OR rows and delete the second row. The first row has invalid data and the second row has valid data. The first row is deleted and the second row shifts up correctly.', () => {
          ...
          (0, _create_new_rule.getIndicatorDeleteButton)().click();
          (0, _create_new_rule.getIndicatorIndexComboField)().should('text', _rule.newThreatIndicatorRule.indicatorMappingField);
    ...
    describe('Generating signals', () => {
      ...
      it('Creates and activates a new Indicator Match rule', () => {
        ...
        cy.get(_rule_details.DEFINITION_DETAILS).within(() => {
          ...
          (0, _rule_details.getDetails)(_rule_details.INDICATOR_MAPPING).should('have.text', `${_rule.newThreatIndicatorRule.indicatorMappingField} MATCHES ${_rule.newThreatIndicatorRule.indicatorIndexField}`);
    ...
    describe('Enrichment', () => {
      ...
      const fields = ['threat.indicator.matched.atomic', 'threat.indicator.matched.type', 'threat.indicator.matched.field'];
      const expectedFieldsText = [_rule.newThreatIndicatorRule.atomic, _rule.newThreatIndicatorRule.type, _rule.newThreatIndicatorRule.indicatorMappingField];
      ...
      it('Displays matches on the timeline', () => {
        (0, _rule_details2.addsFieldsToTimeline)(fieldSearch, fields);
        fields.forEach((field, index) => {
          cy.get((0, _rule_details.TIMELINE_FIELD)(field)).should('have.text', expectedFieldsText[index]);
        });
      });
      it('Displays enrichment on the JSON view', () => {
        (0, _alerts2.expandFirstAlert)();
        (0, _alerts_details2.openJsonView)();
        (0, _alerts_details2.scrollJsonViewToBottom)();
        cy.get(_alerts_details.JSON_LINES).then(elements => {
          const length = elements.length;
          expectedEnrichment.forEach(enrichment => {
            cy.wrap(elements).eq(length - enrichment.line).should('have.text', enrichment.text);
          });
        });
      });
    });
    describe('Duplicates the indicator rule', () => {
      ...
      it('Allows the rule to be duplicated from the table', () => {
        ...
        (0, _edit_rule.goBackToRuleDetails)();
        (0, _rule_details2.goBackToAllRulesTable)();
        (0, _alerts_detection_rules2.checkDuplicatedRule)();
      });
      it("Allows the rule to be duplicated from the table's bulk actions", () => {
        (0, _edit_rule.waitForKibana)();
        (0, _alerts_detection_rules2.selectNumberOfRules)(1);
        (0, _alerts_detection_rules2.duplicateSelectedRules)();
        (0, _alerts_detection_rules2.checkDuplicatedRule)();
      });
