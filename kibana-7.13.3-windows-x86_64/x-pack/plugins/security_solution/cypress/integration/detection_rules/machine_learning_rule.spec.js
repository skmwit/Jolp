...
describe('Detection rules, machine learning', () => {
  ...
  it('Creates and activates a new ml rule', () => {
    ...
    cy.get(_rule_details.DEFINITION_DETAILS).within(() => {
      ...
      _rule.machineLearningRule.machineLearningJobs.forEach((machineLearningJob, jobIndex) => {
        cy.get(_rule_details.MACHINE_LEARNING_JOB_STATUS).eq(jobIndex).should('have.text', 'Stopped');
        cy.get(_rule_details.MACHINE_LEARNING_JOB_ID).eq(jobIndex).should('have.text', machineLearningJob);
      });
    });
