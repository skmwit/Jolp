...
require('../../../../../src/setup_node_env'); // eslint-disable-next-line import/no-extraneous-dependencies


const extract = require('extract-zip');

const fs = require('fs'); // eslint-disable-next-line import/no-extraneous-dependencies


const yaml = require('js-yaml');

const https = require('https'); // eslint-disable-next-line import/no-extraneous-dependencies
...
const {
  ...
  isString,
  pick
} = require('lodash'); // eslint-disable-next-line import/no-extraneous-dependencies


const Q = require('q'); // eslint-disable-next-line import/no-extraneous-dependencies
...
const {
  resolve
} = require('path'); // eslint-disable-next-line import/no-extraneous-dependencies
...
const convertFieldsToHash = (schemaFields, beatFields, path) => schemaFields.fields && isArray(schemaFields.fields) ? schemaFields.fields.reduce((accumulator, item) => {
  if (item.name) {
...
const manageZipFields = async (beat, filePath, beatFields) => {
  try {
    ...
    const obj = yaml.load(fs.readFileSync(`${beat.outputDir}/winlogbeat-${BEATS_VERSION}-windows-x86_64/fields.yml`, {
      encoding: 'utf-8'
    }));
...
const manageTarFields = async (beat, filePath, beatFields) => new Promise((resolve, reject) => {
  fs.createReadStream(filePath).pipe(zlib.createGunzip()).pipe(tar.extract({
    sync: true,
    ...
    console.log('building fields', beat.index);
    const obj = yaml.load(fs.readFileSync(`${beat.outputDir}/fields.yml`, {
      encoding: 'utf-8'
    }));
    const ebeatFields = convertSchemaToHash(obj, beatFields);
    console.log('deleting files', beat.index);
