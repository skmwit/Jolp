...
const deleteNote = async (request, noteIds) => {
  ...
  await Promise.all(noteIds.map(noteId => savedObjectsClient.delete(_notes.noteSavedObjectType, noteId)));
};
  ...
  try {
    ...
    if (noteId == null) {
      const timelineVersionSavedObject = note.timelineId == null ? await (async () => {
        const timelineResult = (0, _timelines.convertSavedObjectToSavedTimeline)(await savedObjectsClient.create(_saved_object_mappings.timelineSavedObjectType, (0, _timelines.pickSavedTimeline)(null, {}, request.user)));
      ...
      return {
        code: 200,
        message: 'success',
        note: convertSavedObjectToSavedNote(await savedObjectsClient.create(_notes.noteSavedObjectType, overrideOwner ? pickSavedNote(noteId, note, request.user) : note), timelineVersionSavedObject != null ? timelineVersionSavedObject : undefined)
      };
    ...
    return {
      code: 200,
      message: 'success',
      note: convertSavedObjectToSavedNote(await savedObjectsClient.update(_notes.noteSavedObjectType, noteId, overrideOwner ? pickSavedNote(noteId, note, request.user) : note, {
