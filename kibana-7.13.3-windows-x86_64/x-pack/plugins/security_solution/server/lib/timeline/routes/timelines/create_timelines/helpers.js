...
const createTimelines = async ({
  ...
  const responseTimeline = await _saved_object.timeline.persistTimeline(frameworkRequest, timelineSavedObjectId, timelineVersion, { ...timeline,
    dateRange: {
  ...
  if (!(0, _fp.isEmpty)(notes)) {
    myPromises = [...myPromises, (0, _persist_notes.persistNotes)(frameworkRequest, timelineSavedObjectId !== null && timelineSavedObjectId !== void 0 ? timelineSavedObjectId : newTimelineSavedObjectId, newTimelineVersion, existingNoteIds, notes, overrideNotesOwner)];
  }
