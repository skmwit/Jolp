...
const buildBulkBody = (ruleSO, doc) => {
  const rule = (0, _build_rule.buildRuleWithOverrides)(ruleSO, doc._source);
...
const buildSignalFromEvent = (event, ruleSO, applyOverrides) => {
  const rule = applyOverrides ? // @ts-expect-error @elastic/elasticsearch _source is optional
  (0, _build_rule.buildRuleWithOverrides)(ruleSO, event._source) : (0, _build_rule.buildRuleWithoutOverrides)(ruleSO);
