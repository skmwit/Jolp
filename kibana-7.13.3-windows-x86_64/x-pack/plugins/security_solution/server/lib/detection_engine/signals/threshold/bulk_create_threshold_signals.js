...
const getTransformedHits = (results, inputIndex, startedAt, from, logger, threshold, ruleId, filter, timestampOverride, thresholdSignalHistory) => {
  const aggParts = threshold.field.length ? results.aggregations && (0, _utils.getThresholdAggregationParts)(results.aggregations) : {
    field: null,
    index: 0,
    name: 'threshold_0'
  };
...
const bulkCreateThresholdSignals = async params => {
  ...
  const ecsResults = transformThresholdResultsToEcs(thresholdResults, params.inputIndexPattern.join(','), params.startedAt, params.from, params.filter, params.logger, ruleParams.threshold, ruleParams.ruleId, ruleParams.timestampOverride, params.thresholdSignalHistory);
