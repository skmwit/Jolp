...
const deleteIndexRoute = router => {
  router.delete({
    ...
    try {
      ...
      const indexExists = await (0, _get_index_exists.getIndexExists)(esClient, index);
      ...
      if (!indexExists) {
        ...
        await (0, _delete_all_index.deleteAllIndex)(esClient, `${index}-*`);
        const policyExists = await (0, _get_policy_exists.getPolicyExists)(esClient, index);

        if (policyExists) {
          await (0, _delete_policy.deletePolicy)(esClient, index);
        }

        const templateExists = await (0, _get_template_exists.getTemplateExists)(esClient, index);

        if (templateExists) {
          await (0, _delete_template.deleteTemplate)(esClient, index);
        }
