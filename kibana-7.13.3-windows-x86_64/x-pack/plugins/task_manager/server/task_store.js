...
class TaskStore {
  ...
  async search(opts = {}) {
    ...
    try {
      ...
      return {
        docs: tasks // @ts-expect-error @elastic/elasticsearch `Hid._id` expected to be `string`
        .filter(doc => this.serializer.isRawSavedObject(doc)) // @ts-expect-error @elastic/elasticsearch `Hid._id` expected to be `string`
