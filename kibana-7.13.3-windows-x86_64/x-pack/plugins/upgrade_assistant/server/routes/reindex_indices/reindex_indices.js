...
function createReindexWorker({
  ...
  return new _reindexing.ReindexWorker(savedObjects, credentialStore, esClient, logger, licensing);
}
...
function registerReindexIndicesRoutes({
  ...
  const BASE_PATH = `${_constants.API_BASE_PATH}/reindex`; // Start reindex for an index
