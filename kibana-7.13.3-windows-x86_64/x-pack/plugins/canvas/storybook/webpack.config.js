...
const {
  DLL_OUTPUT,
  KIBANA_ROOT
} = require('./constants'); // Extend the Storybook Webpack config with some customizations
...
module.exports = async ({
  ...
  const config = {
    module: {
      rules: [// Include the React preset from Kibana for JS(X) and TS(X)
      {
        test: /\.(j|t)sx?$/,
        ...
        use: [{
          ...
          options: {
            prependData(loaderContext) {
              return `@import ${stringifyRequest(loaderContext, path.resolve(KIBANA_ROOT, 'src/core/public/core_app/styles/_globals_v7light.scss'))};\n`;
            },
      ...
      {
        test: [path.resolve(__dirname, '../public/components/embeddable_flyout'), path.resolve(__dirname, '../../reporting/public'), path.resolve(__dirname, '../../../../src/plugins/kibana_legacy/public/angular'), path.resolve(__dirname, '../../../../src/plugins/kibana_legacy/public/paginate')],
        use: 'null-loader'
      }]
    },
    plugins: [// Reference the built DLL file of static(ish) dependencies, which are removed
