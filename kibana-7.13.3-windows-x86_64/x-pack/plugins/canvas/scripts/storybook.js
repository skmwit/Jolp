...
const {
  run
} = require('@kbn/dev-utils'); // This is included in the main Kibana package.json
...
run(({
  ...
  const {
    ...
    stats,
    site
  } = flags; // Delete the existing DLL if we're cleaning or building.
  ...
  if (fs.existsSync(DLL_OUTPUT)) {
    ...
    execa.sync('yarn', ['webpack', '--config', 'x-pack/plugins/canvas/storybook/webpack.dll.config.js', ...(process.stdout.isTTY && !process.env.CI ? ['--progress'] : []), '--hide-modules', '--display-entrypoints', 'false'], {
