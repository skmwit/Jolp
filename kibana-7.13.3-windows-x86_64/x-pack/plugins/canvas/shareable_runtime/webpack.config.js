...
const {
  stringifyRequest
} = require('loader-utils'); // eslint-disable-line
...
module.exports = {
  ...
  plugins: isProd ? [new webpack.optimize.LimitChunkCountPlugin({
    maxChunks: 1
  })] : [],
  output: {
  ...
  resolve: {
    alias: {
      ...
      'data/interpreter': path.resolve(KIBANA_ROOT, 'src/plugins/data/public/expressions/interpreter'),
  ...
  module: {
    rules: [{
      ...
      test: [require.resolve('@elastic/eui/es/components/code_editor'), require.resolve('@elastic/eui/es/components/drag_and_drop'), require.resolve('@elastic/eui/packages/react-datepicker'), require.resolve('highlight.js')],
