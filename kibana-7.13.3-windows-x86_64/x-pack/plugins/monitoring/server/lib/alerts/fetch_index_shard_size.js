...
async function fetchIndexShardSize(esClient, clusters, index, threshold, shardIndexPatterns, size) {
  ...
  const {
    body: response
  } = await esClient.search(params); // @ts-expect-error @elastic/elasticsearch Aggregate does not specify buckets
