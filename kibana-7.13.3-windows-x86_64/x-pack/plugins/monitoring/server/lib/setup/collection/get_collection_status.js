...
async function detectProducts(req, isLiveCluster) {
  const result = {
    [_constants.KIBANA_SYSTEM_ID]: {
      doesExist: true
    },
    [_constants.ELASTICSEARCH_SYSTEM_ID]: {
      doesExist: true
    },
    [_constants.BEATS_SYSTEM_ID]: {
      mightExist: false
    },
    [_constants.APM_SYSTEM_ID]: {
      mightExist: false
    },
    [_constants.LOGSTASH_SYSTEM_ID]: {
      mightExist: false
    }
  };
  const detectionSearch = [{
    id: _constants.BEATS_SYSTEM_ID,
    indices: ['*beat-*', '.management-beats*']
  }, {
    id: _constants.LOGSTASH_SYSTEM_ID,
    indices: ['logstash-*', '.logstash*']
  }, {
    id: _constants.APM_SYSTEM_ID,
    indices: ['apm-*']
  }];
...
function getUuidBucketName(productName) {
  switch (productName) {
    case _constants.ELASTICSEARCH_SYSTEM_ID:
      return 'es_uuids';
...
function isBeatFromAPM(bucket) {
  const beatType = (0, _lodash.get)(bucket, 'beat_type');
...
function shouldSkipBucket(product, bucket) {
  if (product.name === _constants.BEATS_SYSTEM_ID && isBeatFromAPM(bucket)) {
    return true;
  }

  if (product.name === _constants.APM_SYSTEM_ID && !isBeatFromAPM(bucket)) {
    return true;
  }
...
const getCollectionStatus = async (req, indexPatterns, clusterUuid, nodeUuid, skipLiveData) => {
  ...
  const PRODUCTS = [{
    name: _constants.KIBANA_SYSTEM_ID
  }, {
    name: _constants.BEATS_SYSTEM_ID
  }, {
    name: _constants.LOGSTASH_SYSTEM_ID
  }, {
    name: _constants.APM_SYSTEM_ID,
    token: '-beats-'
  }, {
    name: _constants.ELASTICSEARCH_SYSTEM_ID,
    token: '-es-'
  }];
  ...
  const status = PRODUCTS.reduce((products, product) => {
    ...
    const partiallyMigratedUuidsMap = {}; // If there is no data, then they are a net new user
      ...
      productStatus.totalUniqueInstanceCount = 0;
        ...
        for (const bucket of uuidBuckets) {
          ...
          if (!map[key]) {
            ...
            if (product.name === _constants.KIBANA_SYSTEM_ID && key === kibanaUuid) {
              map[key].isPrimary = true;
            }

            if (product.name === _constants.BEATS_SYSTEM_ID) {
              map[key].beatType = (0, _lodash.get)(bucket.beat_type, 'buckets[0].key');
            }

            if (bucket.cluster_uuid) {
              map[key].clusterUuid = (0, _lodash.get)(bucket.cluster_uuid, 'buckets[0].key', '') || null;
            }
          }
        }
        ...
        productStatus.byUuid = { ...productStatus.byUuid,
          ...Object.keys(internalCollectorsUuidsMap).reduce((accum, uuid) => ({ ...accum,
            [uuid]: { ...internalCollectorsUuidsMap[uuid],
              ...productStatus.byUuid[uuid],
              isInternalCollector: true,
              isNetNewUser: false
            }
          }), {}),
          ...Object.keys(partiallyMigratedUuidsMap).reduce((accum, uuid) => ({ ...accum,
            [uuid]: { ...partiallyMigratedUuidsMap[uuid],
              ...productStatus.byUuid[uuid],
              isPartiallyMigrated: true,
              isNetNewUser: false
            }
          }), {}),
          ...Object.keys(fullyMigratedUuidsMap).reduce((accum, uuid) => ({ ...accum,
            [uuid]: { ...fullyMigratedUuidsMap[uuid],
              ...productStatus.byUuid[uuid],
          ...
          const considerAllInstancesMigrated = product.name === _constants.ELASTICSEARCH_SYSTEM_ID && clusterUuid === liveClusterUuid && !liveClusterInternalCollectionEnabled;
    ...
    if (product.name === _constants.ELASTICSEARCH_SYSTEM_ID && liveEsNodes.length) {
    ...
    if (product.name === _constants.KIBANA_SYSTEM_ID && liveKibanaInstance) {
