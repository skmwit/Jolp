...
class MonitoringPlugin {
  ...
  setup(core, plugins) {
    ...
    const cluster = this.cluster = (0, _instantiate_client.instantiateClient)(config.ui.elasticsearch, this.log, core.elasticsearch.legacy.createClient);
    ...
    for (const alert of alerts) {
      ...
      (_plugins$alerting = plugins.alerting) === null || _plugins$alerting === void 0 ? void 0 : _plugins$alerting.registerType(alert.getAlertType());
    ...
    if (config.ui.enabled) {
      ...
      this.monitoringCore = this.getLegacyShim(config, legacyConfig, core.getStartServices, this.cluster, plugins);
  ...
  async start(core, {
    ...
    const config = (0, _config.createConfig)(this.initializerContext.config.get()); // Start our license service which will ensure
  ...
  getLegacyShim(config, legacyConfig, getCoreServices, cluster, setupPlugins) {
