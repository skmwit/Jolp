...
class MissingMonitoringDataAlert extends _base_alert.BaseAlert {
  ...
  async fetchData(params, esClient, clusters, availableCcs) {
    ...
    const now = +new Date();
    const missingData = await (0, _fetch_missing_monitoring_data.fetchMissingMonitoringData)(esClient, clusters, indexPattern, _static_globals.Globals.app.config.ui.max_bucket_size, now, now - limit - LIMIT_BUFFER);
