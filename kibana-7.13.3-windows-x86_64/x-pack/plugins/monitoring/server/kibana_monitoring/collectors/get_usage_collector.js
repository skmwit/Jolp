...
function getMonitoringUsageCollector(usageCollection, config, legacyEsClient) {
  return usageCollection.makeUsageCollector({
    ...
    fetch: async ({
      ...
      const usageClusters = [];
      const availableCcs = config.ui.ccs.enabled ? await (0, _fetch_available_ccs.fetchAvailableCcsLegacy)(callCluster) : [];
      const elasticsearchIndex = (0, _get_ccs_index_pattern.getCcsIndexPattern)(_constants.INDEX_PATTERN_ELASTICSEARCH, availableCcs);
      const clusters = await (0, _fetch_clusters.fetchClustersLegacy)(callCluster, elasticsearchIndex);
