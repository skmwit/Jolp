...
function getTemplate({
  ...
  const template = getBaseTemplate(type, templateIndexPattern, fields, mappings, packageName, composedOfTemplates, templatePriority, ilmPolicy, hidden);
...
function getBaseTemplate(type, templateIndexPattern, fields, mappings, packageName, composedOfTemplates, templatePriority, ilmPolicy, hidden) {
  const logger = _.appContextService.getLogger(); // Meta information to identify Ingest Manager's managed templates and indices
  ...
  if (defaultFields.length > QUERY_DEFAULT_FIELD_LIMIT) {
    logger.warn(`large amount of default fields detected for index template ${templateIndexPattern} in package ${packageName}, applying the first ${QUERY_DEFAULT_FIELD_LIMIT} fields`);
  }
