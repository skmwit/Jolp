...
class PackagePolicyService {
  async create(soClient, esClient, packagePolicy, options) {
    ...
    if (parentAgentPolicy.is_managed && !(options !== null && options !== void 0 && options.force)) {
      throw new _errors.HostedAgentPolicyRestrictionRelatedError(`Cannot add integrations to hosted agent policy ${parentAgentPolicy.id}`);
    }

    if (parentAgentPolicy.package_policies.find(siblingPackagePolicy => siblingPackagePolicy.name === packagePolicy.name)) {
      throw new _errors.IngestManagerError('There is already a package with the same name on this agent policy');
    } // Add ids to stream
