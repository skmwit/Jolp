...
const evaluateAlert = (esClient, params, config, timeframe) => {
  const {
    criteria,
    groupBy,
    filterQuery
  ...
  return Promise.all(criteria.map(async criterion => {
    const currentValues = await getMetric(esClient, criterion, config.metricAlias, config.fields.timestamp, groupBy, filterQuery, timeframe);
    const {
      threshold,
      warningThreshold,
      comparator,
...
const getMetric = async function (esClient, params, index, timefield, groupBy, filterQuery, timeframe) {
  const {
    aggType
  } = params;
...
const getValuesFromAggregations = (aggregations, aggType) => {
  try {
    ...
    return buckets.map(bucket => {
      ...
      return {
        key: (_bucket$key_as_string = bucket.key_as_string) !== null && _bucket$key_as_string !== void 0 ? _bucket$key_as_string : bucket.to_as_string,
