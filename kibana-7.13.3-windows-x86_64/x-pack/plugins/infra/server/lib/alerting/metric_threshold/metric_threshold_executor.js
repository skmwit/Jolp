...
const createMetricThresholdExecutor = libs => async function (options) {
  ...
  const alertResults = await (0, _evaluate_alert.evaluateAlert)(services.scopedClusterClient.asCurrentUser, params, config); // Because each alert result has the same group definitions, just grab the groups from the first one.
