  ...
  return ({
    ...
    return {
      async run() {
        ...
        return Promise.all([(0, _alerts_telemetry.getTotalCountAggregations)(esClient, kibanaIndex), (0, _alerts_telemetry.getTotalCountInUse)(esClient, kibanaIndex)]).then(([totalCountAggregations, totalInUse]) => {
          return {
            state: {
              runs: (state.runs || 0) + 1,
