...
async function getTotalCountAggregations(esClient, kibanaInex) {
  ...
  const connectorsMetric = {
    scripted_metric: {
      init_script: 'state.currentAlertActions = 0; state.min = 0; state.max = 0; state.totalActionsCount = 0;',
      map_script: `
  ...
  const totalAlertsCount = Object.keys(aggregations.byAlertTypeId.value.types).reduce((total, key) => parseInt(aggregations.byAlertTypeId.value.types[key], 0) + total, 0);
  return {
    count_total: totalAlertsCount,
    count_by_type: Object.keys(aggregations.byAlertTypeId.value.types).reduce( // ES DSL aggregations are returned as `any` by esClient.search
    ...
    throttle_time: {
      ...
      avg: `${aggregations.throttleTime.value.totalCount > 0 ? aggregations.throttleTime.value.totalSum / aggregations.throttleTime.value.totalCount : 0}s`,
      max: `${aggregations.throttleTime.value.max}s`
    },
    schedule_time: {
      min: `${aggregations.intervalTime.value.min}s`,
      avg: `${aggregations.intervalTime.value.totalCount > 0 ? aggregations.intervalTime.value.totalSum / aggregations.intervalTime.value.totalCount : 0}s`,
      max: `${aggregations.intervalTime.value.max}s`
    },
    connectors_per_alert: {
      min: aggregations.connectorsAgg.connectors.value.min,
      avg: totalAlertsCount > 0 ? aggregations.connectorsAgg.connectors.value.totalActionsCount / totalAlertsCount : 0,
...
async function getTotalCountInUse(esClient, kibanaInex) {
  ...
  return {
    countTotal: Object.keys(aggregations.byAlertTypeId.value.types).reduce((total, key) => parseInt(aggregations.byAlertTypeId.value.types[key], 0) + total, 0),
    countByType: Object.keys(aggregations.byAlertTypeId.value.types).reduce( // ES DSL aggregations are returned as `any` by esClient.search
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (obj, key) => ({ ...obj,
      [replaceFirstAndLastDotSymbols(key)]: aggregations.byAlertTypeId.value.types[key]
    }), {})
