...
class AlertingPlugin {
  ...
  setup(core, plugins) {
    ...
    core.getStartServices().then(async ([coreStart, startPlugins]) => {
      (0, _rxjs.combineLatest)([core.status.derivedStatus$, (0, _health.getHealthStatusStream)(startPlugins.taskManager, this.logger, coreStart.savedObjects, this.config)]).pipe((0, _operators.map)(([derivedStatus, healthStatus]) => {
        if (healthStatus.level > derivedStatus.level) {
          return healthStatus;
        } else {
          return derivedStatus;
        }
      }), (0, _operators.share)()).subscribe(serviceStatus$);
    });
    ...
    (0, _routes.defineRoutes)(router, this.licenseState, plugins.encryptedSavedObjects);
    return {
      registerType(alertType) {
        if (!(alertType.minimumLicenseRequired in _server2.LICENSE_TYPE)) {
