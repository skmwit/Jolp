...
async function deleteSubCases({
  ...
  await Promise.all(commentsForSubCases.saved_objects.map(commentSO => caseService.deleteComment({
    client,
    commentId: commentSO.id
  })));
  await Promise.all(subCasesForCaseIds.saved_objects.map(subCaseSO => caseService.deleteSubCase(client, subCaseSO.id)));
}
    ...
    try {
      ...
      await userActionService.postUserActions({
        ...
        actions: request.query.ids.map(id => (0, _helpers.buildCaseUserActionItem)({
          ...
          caseId: id,
          fields: ['comment', 'description', 'status', 'tags', 'title', ...(_constants.ENABLE_CASE_CONNECTOR ? ['sub_case'] : [])]
        }))
      });
