...
function initFindCaseCommentsApi({
  ...
  router.get({
    ...
    try {
      ...
      if (!_constants.ENABLE_CASE_CONNECTOR && query.subCaseId !== undefined) {
        throw _boom.default.badRequest('The `subCaseId` is not supported when the case connector feature is disabled');
      }

      const id = (_query$subCaseId = query.subCaseId) !== null && _query$subCaseId !== void 0 ? _query$subCaseId : request.params.case_id;
      const associationType = query.subCaseId ? _api.AssociationType.subCase : _api.AssociationType.case;
      const args = query ? {
        caseService,
        client,
        id,
