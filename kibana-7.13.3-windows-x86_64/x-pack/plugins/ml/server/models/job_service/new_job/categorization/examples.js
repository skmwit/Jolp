...
exports.categorizationExamplesProvider = categorizationExamplesProvider;

var _lodash = require("lodash");

var _categorization_job = require("../../../../../common/constants/categorization_job");

var _validation_results = require("./validation_results");
/*
...
function categorizationExamplesProvider({
  asCurrentUser,
  asInternalUser
}) {
  const validationResults = new _validation_results.ValidationResults();

  async function categorizationExamples(indexPatternTitle, query, size, categorizationFieldName, timeField, start, end, analyzer, runtimeMappings, indicesOptions) {
    ...
    const tempExamples = body.hits.hits.map(({
      fields
    }) => fields && Array.isArray(fields[categorizationFieldName]) && fields[categorizationFieldName].length > 0 ? fields[categorizationFieldName][0] : null);
    validationResults.createNullValueResult(tempExamples);
    const allExamples = tempExamples.filter(example => example !== undefined && example !== null);
  ...
  async function getTokens(chunkSize, examples, analyzer) {
    const exampleChunks = (0, _lodash.chunk)(examples, chunkSize);
    const tokensPerExampleChunks = [];
    ...
    return examples.map((e, i) => ({
      text: e,
      tokens: tokensPerExample[i]
    }));
  }

  async function loadTokens(examples, analyzer) {
    const {
      body: {
        tokens
      }
    } = await asInternalUser.indices.analyze({
      body: { ...getAnalyzer(analyzer),
        text: examples
      }
    });
    const lengths = examples.map(e => e.length);
    const sumLengths = lengths.map((s => a => s += a)(0));
    const tokensPerExample = examples.map(e => []);
  ...
  async function validateCategoryExamples(indexPatternTitle, query, size, categorizationFieldName, timeField, start, end, analyzer, runtimeMappings, indicesOptions) {
    const resp = await categorizationExamples(indexPatternTitle, query, _categorization_job.CATEGORY_EXAMPLES_SAMPLE_SIZE, categorizationFieldName, timeField, start, end, analyzer, runtimeMappings, indicesOptions);
    const {
      examples
    } = resp;
    const sampleSize = examples.length;
    validationResults.createTokenCountResult(examples, sampleSize); // sort examples by number of tokens, keeping track of their original order
    // with an origIndex property

    const sortedExamples = examples.map((e, i) => ({ ...e,
      origIndex: i
    })).sort((a, b) => b.tokens.length - a.tokens.length); // we only want 'size' (e.g. 5) number of examples,
    ...
    const processedExamples = filteredExamples.sort((a, b) => a.origIndex - b.origIndex).map(e => ({
      text: e.text,
      tokens: e.tokens
    }));
    return {
