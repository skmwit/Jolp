...
function jobsProvider(client, mlClient, alertsClient) {
  const {
    asInternalUser
  } = client;
  const {
  ...
  async function jobsSummary(jobIds = []) {
    ...
    const jobs = fullJobsList.map(job => {
      ...
      const tempJob = {
        ...
        earliestStartTimestampMs: (0, _job_utils.getEarliestDatafeedStartTime)(dataCounts === null || dataCounts === void 0 ? void 0 : dataCounts.latest_record_timestamp, // @ts-expect-error @elastic/elasticsearch data counts missing is missing latest_bucket_timestamp
        dataCounts === null || dataCounts === void 0 ? void 0 : dataCounts.latest_bucket_timestamp, (0, _job_utils.parseTimeIntervalForJob)((_job$analysis_config = job.analysis_config) === null || _job$analysis_config === void 0 ? void 0 : _job$analysis_config.bucket_span)),
        ...
        latestResultsTimestampMs: (0, _job_utils.getLatestDataOrBucketTimestamp)(dataCounts === null || dataCounts === void 0 ? void 0 : dataCounts.latest_record_timestamp, // @ts-expect-error @elastic/elasticsearch data counts missing is missing latest_bucket_timestamp
        dataCounts === null || dataCounts === void 0 ? void 0 : dataCounts.latest_bucket_timestamp),
  ...
  async function jobsWithTimerange() {
    ...
    const jobs = fullJobsList.map(job => {
      ...
      if (dataCounts !== undefined) {
        timeRange.to = (0, _job_utils.getLatestDataOrBucketTimestamp)(dataCounts.latest_record_timestamp, // @ts-expect-error @elastic/elasticsearch data counts missing is missing latest_bucket_timestamp
  ...
  async function createFullJobsList(jobIds = []) {
    ...
    if (jobResults && jobResults.jobs) {
      ...
      if (alertsClient) {
        const mlAlertingRules = await alertsClient.find({
          options: {
