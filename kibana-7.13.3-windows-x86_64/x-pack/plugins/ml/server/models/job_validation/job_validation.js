...
async function validateJob(client, mlClient, payload, isSecurityDisabled) {
  ...
  try {
    ...
    if (basicValidation.valid === true) {
      ...
      if (typeof duration === 'undefined' && (await (0, _validate_time_range.isValidTimeField)(client, job))) {
        ...
        const timeRange = await fs.getTimeFieldRange(index, timeField, job.datafeed_config.query, job.datafeed_config.runtime_mappings, // @ts-expect-error @elastic/elasticsearch Datafeed is missing indices_options
