...
const runTaskFnFactory = function executeJobFactoryFn(reporting, parentLogger) {
  ...
  return async function runTask(jobId, job, cancellationToken) {
    ...
    const {
      ...
      csvContainsFormulas,
      warnings
    } = await generateCsv(job, config, uiSettingsClient, elasticsearchClient, cancellationToken); // @TODO: Consolidate these one-off warnings into the warnings array (max-size reached and csv contains formulas)
