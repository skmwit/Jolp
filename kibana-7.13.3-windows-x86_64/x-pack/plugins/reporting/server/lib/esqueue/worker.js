...
exports.formatJobObject = formatJobObject;
exports.getUpdatedDocPath = getUpdatedDocPath;
...
function getUpdatedDocPath(response) {
  const {
    _index: ind,
    _type: type = '_doc',
    _id: id
  ...
  return (msg, err) => {
    ...
    if (err) {
      ...
      if (errLength > MAX_ERROR_LENGTH) {
        const partialError = subStr(0, MAX_PARTIAL_ERROR_LENGTH) + ERROR_PARTIAL_SEPARATOR + subStr(errLength - MAX_PARTIAL_ERROR_LENGTH);
        logger(partialError, tags);
        logger(`A partial version of the entire error message was logged. ` + `The entire error message length is: ${errLength} characters.`, tags);
      } else {
...
class Worker extends _events.default.EventEmitter {
  constructor(queue, type, workerFn, opts) {
    ...
    this._poller = new _common.Poller({
      functionToPoll: () => {
  ...
  _claimJob(job) {
    ...
    const doc = {
      ...
      status: _constants.constants.JOB_STATUS_PROCESSING,
    ...
    return this.queue.store.setReportClaimed(job, doc).then(response => {
      ...
      updatedJob._source = { ...job._source,
        ...doc
      };
      return updatedJob;
    });
  ...
  _failJob(job, output = false) {
    ...
    const doc = {
      status: _constants.constants.JOB_STATUS_FAILED,
      completed_at: completedTime,
      output: docOutput
    };
    this.emit(_constants.constants.EVENT_WORKER_JOB_FAIL, {
      job: formatJobObject(job),
      worker: this.toJSON(),
      output: docOutput
    });
    return this.queue.store.setReportFailed(job, doc).then(response => {
      this.info(`Job marked as failed: ${getUpdatedDocPath(response)}`);
    }).catch(err => {
      if (err.statusCode === 409) return true;
      this.error(`_failJob failed to update job ${job._id}`, err);
      this.emit(_constants.constants.EVENT_WORKER_FAIL_UPDATE_ERROR, this._formatErrorParams(err, job));
      return false;
    });
  }
  ...
  _performJob(job) {
    ...
    const workerOutput = new Promise((resolve, reject) => {
      ...
      const cancellationToken = new _common.CancellationToken();
      const jobSource = job._source;
      Promise.resolve(this.workerFn.call(null, job, jobSource.payload, cancellationToken)).then(res => {
      ...
      setTimeout(() => {
        ...
        reject(new _errors.WorkerTimeoutError(`Worker timed out, timeout = ${timeout}`, {
          jobId: job._id,
          timeout
        }));
      }, timeout);
    ...
    return workerOutput.then(output => {
      ...
      const status = output && output.warnings && output.warnings.length > 0 ? _constants.constants.JOB_STATUS_WARNINGS : _constants.constants.JOB_STATUS_COMPLETED;
      const doc = {
        status,
        completed_at: completedTime,
        output: docOutput
      };
      return this.queue.store.setReportCompleted(job, doc).then(response => {
        const eventOutput = {
          job: formatJobObject(job),
          output: docOutput
        };
        this.emit(_constants.constants.EVENT_WORKER_COMPLETE, eventOutput);
        this.info(`Job data saved successfully: ${getUpdatedDocPath(response)}`);
      }).catch(err => {
        if (err.statusCode === 409) return false;
        this.error(`Failure saving job output ${job._id}`, err);
        this.emit(_constants.constants.EVENT_WORKER_JOB_UPDATE_ERROR, this._formatErrorParams(err, job));
        return this._failJob(job, err.message ? err.message : false);
      });
    }, jobErr => {
      if (!jobErr) {
        jobErr = new _errors.UnspecifiedWorkerError('Unspecified worker error', {
          jobId: job._id
        });
      ...
      if (jobErr.name === 'WorkerTimeoutError') {
        ...
        this.emit(_constants.constants.EVENT_WORKER_JOB_TIMEOUT, this._formatErrorParams(jobErr, job));
        return; // append the jobId to the error
      } else {
        try {
          Object.assign(jobErr, {
            jobId: job._id
          });
        } catch (e) {// do nothing if jobId can not be appended
        }
      }

      this.error(`Failure occurred on job ${job._id}`, jobErr);
      this.emit(_constants.constants.EVENT_WORKER_JOB_EXECUTION_ERROR, this._formatErrorParams(jobErr, job));
  ...
  _claimPendingJobs(jobs) {
    ...
    let claimed = false; // claim a single job, stopping after first successful claim
    ...
    return jobs.reduce((chain, job) => {
      return chain.then(claimedJob => {
        ...
        return this._claimJob(job).then(claimResult => {
          claimed = true;
          return claimResult;
        }).catch(err => {
          ...
          this.emit(_constants.constants.EVENT_WORKER_JOB_CLAIM_ERROR, this._formatErrorParams(err, job));
          return Promise.reject(err);
        });
      });
    }, Promise.resolve()).then(claimedJob => {
      if (!claimedJob) {
  ...
  _getPendingJobs() {
    const nowTime = (0, _moment.default)().toISOString();
    const query = {
      ...
      size: _constants.constants.DEFAULT_WORKER_CHECK_SIZE
    };
    return this._client.search({
      ...
      this.emit(_constants.constants.EVENT_WORKER_JOB_SEARCH_ERROR, this._formatErrorParams(err));
      throw err;
    });
  }
