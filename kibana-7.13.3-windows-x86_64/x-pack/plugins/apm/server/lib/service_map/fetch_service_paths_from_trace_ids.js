...
exports.fetchServicePathsFromTraceIds = fetchServicePathsFromTraceIds;
...
async function fetchServicePathsFromTraceIds(setup, traceIds) {
  return (0, _with_apm_span.withApmSpan)('get_service_paths_from_trace_ids', async () => {
    const {
      apmEventClient
    } = setup; // make sure there's a range so ES can skip shards
      ...
      body: {
        ...
        query: {
          bool: {
            filter: [{
              terms: {
                [_elasticsearch_fieldnames.TRACE_ID]: traceIds
              }
    ...
    const serviceMapFromTraceIdsScriptResponse = await apmEventClient.search(serviceMapParams);
    return serviceMapFromTraceIdsScriptResponse;
  });
}