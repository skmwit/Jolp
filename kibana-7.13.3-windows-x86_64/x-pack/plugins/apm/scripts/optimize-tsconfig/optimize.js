...
async function prepareBaseTsConfig() {
  ...
  await writeFile(baseConfigFilename, JSON.stringify({ ...omit(config, 'references'),
...
async function addApmFilesToRootTsConfig() {
  ...
  await writeFile(rootTsConfigFilename, JSON.stringify({ ...rootTsConfig,
    ...template,
    references: []
  }, null, 2), {
    encoding: 'utf-8'
...
async function addApmFilesToTestTsConfig() {
  ...
  await writeFile(testTsConfigFilename, JSON.stringify({ ...testTsConfig,
    ...template,
    references: []
  }, null, 2), {
    encoding: 'utf-8'
...
async function optimizeTsConfig() {
  ...
  await setIgnoreChanges(); // eslint-disable-next-line no-console

  console.log('Created an optimized tsconfig.json for APM. To undo these changes, run `./scripts/unoptimize-tsconfig.js`');
}
