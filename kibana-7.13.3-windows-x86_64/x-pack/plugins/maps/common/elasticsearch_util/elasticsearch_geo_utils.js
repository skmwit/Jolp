...
function createExtentFilter(mapExtent, geoFieldName) {
  return {
    geo_bounding_box: {
...
function createSpatialFilterWithGeometry({
  ...
  const meta = {
    ...
    key: geoFieldName,
    alias: `${geoFieldName} ${(0, _i18n_getters.getEsSpatialRelationLabel)(relation)} ${geometryLabel}`,
    disabled: false
  };
  const shapeQuery = {
    relation
...
function extractFeaturesFromFilters(filters) {
  ...
  filters.filter(filter => {
    ...
    } else if (filter.geo_shape && filter.geo_shape[geoFieldName] && filter.geo_shape[geoFieldName].shape) {
