...
function buildMapsSavedObjectsTelemetry(layerLists) {
  ...
  const telemetryLayerTypeCounts = (0, _util.getTelemetryLayerTypesPerCluster)(layerLists);
  const scalingOptions = (0, _util.getScalingOptionsPerCluster)(layerLists);
  const joins = (0, _util.getTermJoinsPerCluster)(layerLists);
  const basemaps = (0, _util.getBaseMapsPerCluster)(layerLists);
  return {
    // Total count of maps
