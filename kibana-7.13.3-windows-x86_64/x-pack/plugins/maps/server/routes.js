...
async function initRoutes(core, getLicenseId, emsSettings, kbnVersion, logger, drawingFeatureEnabled) {
  let emsClient;
  let lastLicenseId;
  ...
  router.get({
    path: `${_common.API_ROOT_PATH}/${_common.EMS_FILES_API_PATH}/${_common.EMS_FILES_DEFAULT_JSON_PATH}`,
    validate: {
  ...
  router.get({
    path: `${_common.API_ROOT_PATH}/${_common.EMS_TILES_API_PATH}/${_common.EMS_TILES_RASTER_TILE_PATH}`,
    validate: false
  ...
  router.get({
    path: `${_common.API_ROOT_PATH}/${_common.EMS_CATALOGUE_PATH}`,
    validate: false
  }, async (context, request, {
    ok,
    badRequest
    ...
    if (tileService) {
      proxiedManifest.services.push({ ...tileService,
        manifest: `${_common.GIS_API_PATH}/${_common.EMS_TILES_CATALOGUE_PATH}`
      });
    }

    if (fileService) {
      proxiedManifest.services.push({ ...fileService,
        manifest: `${_common.GIS_API_PATH}/${_common.EMS_FILES_CATALOGUE_PATH}`
      });
    }
  ...
  router.get({
    path: `${_common.API_ROOT_PATH}/${_common.EMS_FILES_CATALOGUE_PATH}/{emsVersion}/manifest`,
    validate: false
  }, async (context, request, {
    ok,
    badRequest
    ...
    const layers = file.layers.map(layerJson => {
      ...
      const newUrl = `${_common.EMS_FILES_DEFAULT_JSON_PATH}?id=${id}`; //Only proxy default-format. Others are unused in Maps-app
  ...
  router.get({
    path: `${_common.API_ROOT_PATH}/${_common.EMS_TILES_CATALOGUE_PATH}/{emsVersion}/manifest`,
    validate: false
  }, async (context, request, {
    ok,
    badRequest
    ...
    const newServices = tilesManifest.services.map(service => {
      ...
      if (rasterFormats.length) {
        const newUrl = `${_common.EMS_TILES_RASTER_STYLE_PATH}?id=${service.id}`;
      ...
      if (vectorFormats.length) {
        const newUrl = `${_common.EMS_TILES_VECTOR_STYLE_PATH}?id=${service.id}`;
  ...
  router.get({
    path: `${_common.API_ROOT_PATH}/${_common.EMS_TILES_API_PATH}/${_common.EMS_TILES_RASTER_STYLE_PATH}`,
    validate: {
      query: _configSchema.schema.object({
        id: _configSchema.schema.maybe(_configSchema.schema.string())
      })
    ...
    const newUrl = `${_common.EMS_TILES_RASTER_TILE_PATH}?id=${request.query.id}&x={x}&y={y}&z={z}`;
    return ok({
      body: { ...style,
        tiles: [newUrl]
      }
  ...
  router.get({
    path: `${_common.API_ROOT_PATH}/${_common.EMS_TILES_API_PATH}/${_common.EMS_TILES_VECTOR_STYLE_PATH}`,
    validate: {
      query: _configSchema.schema.object({
    ...
    for (const sourceId in vectorStyle.sources) {
      if (vectorStyle.sources.hasOwnProperty(sourceId)) {
        newSources[sourceId] = {
          type: 'vector',
          url: `${_common.EMS_TILES_VECTOR_SOURCE_PATH}?id=${request.query.id}&sourceId=${sourceId}`
        };
      }
    }

    const spritePath = `${_common.EMS_SPRITES_PATH}/${request.query.id}/sprite`;
    return ok({
      body: { ...vectorStyle,
        glyphs: `${_common.EMS_GLYPHS_PATH}/{fontstack}/{range}`,
        sprite: spritePath,
        sources: newSources
      }
    });
  ...
  router.get({
    path: `${_common.API_ROOT_PATH}/${_common.EMS_TILES_API_PATH}/${_common.EMS_TILES_VECTOR_SOURCE_PATH}`,
    validate: {
      query: _configSchema.schema.object({
        id: _configSchema.schema.string(),
    ...
    const newUrl = `${_common.EMS_TILES_VECTOR_TILE_PATH}?id=${request.query.id}&sourceId=${request.query.sourceId}&x={x}&y={y}&z={z}`;
    return ok({
      body: { ...sourceManifest,
        tiles: [newUrl]
      }
  ...
  router.get({
    path: `${_common.API_ROOT_PATH}/${_common.EMS_TILES_API_PATH}/${_common.EMS_TILES_VECTOR_TILE_PATH}`,
    validate: {
      query: _configSchema.schema.object({
        id: _configSchema.schema.string(),
  ...
  router.get({
    path: `${_common.API_ROOT_PATH}/${_common.EMS_TILES_API_PATH}/${_common.EMS_GLYPHS_PATH}/{fontstack}/{range}`,
    validate: {
  ...
  router.get({
    path: `${_common.API_ROOT_PATH}/${_common.EMS_TILES_API_PATH}/${_common.EMS_SPRITES_PATH}/{id}/sprite{scaling?}.{extension}`,
    validate: {
      query: _configSchema.schema.object({
        elastic_tile_service_tos: _configSchema.schema.maybe(_configSchema.schema.string()),
        my_app_name: _configSchema.schema.maybe(_configSchema.schema.string()),
  ...
  router.get({
    path: `/${_common.FONTS_API_PATH}/{fontstack}/{range}`,
    validate: {
      params: _configSchema.schema.object({
        fontstack: _configSchema.schema.string(),
        range: _configSchema.schema.string()
  ...
  router.get({
    path: `/${_common.INDEX_SETTINGS_API_PATH}`,
    validate: {
      query: _configSchema.schema.object({
