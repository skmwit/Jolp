...
const migrateByValuePanels = (deps, version) => doc => {
  const {
    attributes
  ...
  panels.forEach(panel => {
    ...
    if (originalPanelState.explicitInput.attributes || originalPanelState.explicitInput.savedVis) {
      ...
      const migratedInput = deps.embeddable.migrate({ ...originalPanelState.explicitInput,
        type: originalPanelState.type
      }, version); // Convert the embeddable state back into the panel shape
...
const createDashboardSavedObjectTypeMigrations = deps => {
  const embeddableMigrations = deps.embeddable.getMigrationVersions().filter(version => _semver.default.gt(version, '7.12.0')).map(version => {
