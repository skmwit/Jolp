 ...
 */
const insertDataIntoIndex = (dataIndexConfig, index, nowReference, esClient, logger) => {
  ...
  const bulkInsert = async docs => {
    ...
    const {
      body: resp
    } = await esClient.asCurrentUser.bulk({
      body: bulk
    });
...
function createInstallRoute(router, sampleDatasets, logger, usageTracker) {
  router.post({
    ...
    for (let i = 0; i < sampleDataset.dataIndices.length; i++) {
      ...
      try {
        const count = await insertDataIntoIndex(dataIndexConfig, index, nowReference, context.core.elasticsearch.client, logger);
        counts[index] = count;
      } catch (err) {
