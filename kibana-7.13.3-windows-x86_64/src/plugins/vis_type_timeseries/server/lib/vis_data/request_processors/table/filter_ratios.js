...
function ratios(req, panel, esQueryConfig, seriesIndex) {
  return next => doc => {
    panel.series.forEach(column => {
      ...
      if (column.metrics.some(filter)) {
        column.metrics.filter(filter).forEach(metric => {
          (0, _helpers.overwrite)(doc, `${aggRoot}.timeseries.aggs.${metric.id}-numerator.filter`, _server.esQuery.buildEsQuery(seriesIndex.indexPattern, metric.numerator, [], esQueryConfig));
          (0, _helpers.overwrite)(doc, `${aggRoot}.timeseries.aggs.${metric.id}-denominator.filter`, _server.esQuery.buildEsQuery(seriesIndex.indexPattern, metric.denominator, [], esQueryConfig));
