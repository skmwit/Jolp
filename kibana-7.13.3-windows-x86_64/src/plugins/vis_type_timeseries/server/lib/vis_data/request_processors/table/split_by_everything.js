 ...
 */
function splitByEverything(req, panel, esQueryConfig, seriesIndex) {
  return next => doc => {
    panel.series.filter(c => !(c.aggregate_by && c.aggregate_function)).forEach(column => {
      if (column.filter) {
        (0, _helpers.overwrite)(doc, `aggs.pivot.aggs.${column.id}.filter`, _server.esQuery.buildEsQuery(seriesIndex.indexPattern, [column.filter], [], esQueryConfig));
      } else {
