 ...
 */
function siblingBuckets(req, panel, esQueryConfig, seriesIndex, capabilities, uiSettings) {
  return next => async doc => {
    const barTargetUiSettings = await uiSettings.get(_common.UI_SETTINGS.HISTOGRAM_BAR_TARGET);
    const {
      interval
    } = (0, _get_interval_and_timefield.getIntervalAndTimefield)(panel, {}, seriesIndex);
    const {
      bucketSize
