...
class KibanaUsageCollectionPlugin {
  constructor(initializerContext) {
    ...
    _defineProperty(this, "stopUsingUiCounterIndicies$", void 0);

    this.logger = initializerContext.logger.get();
    this.legacyConfig$ = initializerContext.config.legacy.globalConfig$;
    this.metric$ = new _rxjs.Subject();
    this.stopUsingUiCounterIndicies$ = new _rxjs.Subject();
  }
  ...
  setup(coreSetup, {
    ...
    this.registerUsageCollectors(usageCollection, coreSetup, this.metric$, this.stopUsingUiCounterIndicies$, coreSetup.savedObjects.registerType.bind(coreSetup.savedObjects));
  }
  ...
  registerUsageCollectors(usageCollection, coreSetup, metric$, stopUsingUiCounterIndicies$, registerType) {
    ...
    (0, _collectors.registerUiCountersRollups)(this.logger.get('ui-counters'), stopUsingUiCounterIndicies$, getSavedObjectsClient);
    (0, _collectors.registerUiCountersUsageCollector)(usageCollection, stopUsingUiCounterIndicies$);
