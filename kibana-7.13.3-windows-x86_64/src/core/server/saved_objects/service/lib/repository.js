...
class SavedObjectsRepository {
  ...
  async bulkCreate(objects, options = {}) {
    ...
    const expectedBulkResults = expectedResults.map(expectedBulkGetResult => {
      ...
      if (esRequestIndex !== undefined) {
        ...
        const docFound = indexFound && (actualResult === null || actualResult === void 0 ? void 0 : actualResult.found) === true; // @ts-expect-error MultiGetHit._source is optional
        ...
        savedObjectNamespaces = initialNamespaces || // @ts-expect-error MultiGetHit._source is optional
        getSavedObjectNamespaces(namespace, docFound ? actualResult : undefined); // @ts-expect-error MultiGetHit._source is optional
    ...
    return {
      saved_objects: expectedBulkResults.map(expectedResult => {
        ...
        const {
          error,
          ...rawResponse
        } = Object.values((_bulkResponse$body$it = bulkResponse === null || bulkResponse === void 0 ? void 0 : bulkResponse.body.items[esRequestIndex]) !== null && _bulkResponse$body$it !== void 0 ? _bulkResponse$body$it : {})[0];
  ...
  async delete(type, id, options = {}) {
    ...
    const deleteDocNotFound = body.result === 'not_found'; // @ts-expect-error 'error' does not exist on type 'DeleteResponse'
  ...
  async find(options) {
    ...
    return { ...(body.aggregations ? {
      ...
      saved_objects: body.hits.hits.map(hit => ({ // @ts-expect-error @elastic/elasticsearch declared Id as string | number
  ...
  async resolve(type, id, options = {}) {
    var _aliasResponse$body$g, _aliasResponse$body$g2, _aliasResponse$body$g3;
    ...
    if (aliasResponse.statusCode === 404 || ((_aliasResponse$body$g = aliasResponse.body.get) === null || _aliasResponse$body$g === void 0 ? void 0 : _aliasResponse$body$g.found) === false || ((_aliasResponse$body$g2 = aliasResponse.body.get) === null || _aliasResponse$body$g2 === void 0 ? void 0 : (_aliasResponse$body$g3 = _aliasResponse$body$g2._source[_object_types.LEGACY_URL_ALIAS_TYPE]) === null || _aliasResponse$body$g3 === void 0 ? void 0 : _aliasResponse$body$g3.disabled) === true) {
  ...
  async deleteFromNamespaces(type, id, namespaces, options = {}) {
    ...
    if (remainingNamespaces !== null && remainingNamespaces !== void 0 && remainingNamespaces.length) {
      ...
      const deleteDocNotFound = body.result === 'not_found'; // @ts-expect-error
  ...
  async bulkUpdate(objects, options = {}) {
    ...
    return {
      saved_objects: expectedBulkUpdateResults.map(expectedResult => {
        ...
        const response = (_bulkUpdateResponse$b = bulkUpdateResponse === null || bulkUpdateResponse === void 0 ? void 0 : bulkUpdateResponse.body.items[esRequestIndex]) !== null && _bulkUpdateResponse$b !== void 0 ? _bulkUpdateResponse$b : {}; // When a bulk update operation is completed, any fields specified in `_sourceIncludes` will be found in the "get" value of the
  ...
  async openPointInTimeForType(type, {
    ...
    const {
      body,
      statusCode
    } = await this.client.openPointInTime( // @ts-expect-error @elastic/elasticsearch OpenPointInTimeRequest.index expected to accept string[]
    esOptions, {
      ignore: [404]
    });
