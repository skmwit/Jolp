...
async function migrateSourceToDest(context) {
  ...
  while (true) {
    ...
    await Index.write(client, dest.indexName, await (0, _migrate_raw_docs.migrateRawDocs)(serializer, documentMigrator.migrateAndConvert, // @ts-expect-error @elastic/elasticsearch `Hit._id` may be a string | number in ES, but we always expect strings in the SO index.
    docs, log));
  }
}