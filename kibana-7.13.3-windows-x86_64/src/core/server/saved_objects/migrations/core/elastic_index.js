...
exports.excludeUnusedTypesQuery = void 0;
...
const excludeUnusedTypesQuery = {
  bool: {
    must_not: [// https://github.com/elastic/kibana/issues/91869
    {
      term: {
...
async function fetchInfo(client, index) {
  const {
    body,
    statusCode
  ...
  const [indexName, indexInfo] = Object.entries(body)[0]; // @ts-expect-error @elastic/elasticsearch IndexState.alias and IndexState.mappings should be required

  return assertIsSupportedIndex({ ...indexInfo,
    exists: true,
    indexName
  });
}
...
function reader(client, index, {
  ...
  const nextBatch = () => scrollId !== undefined ? client.scroll({
    scroll,
    scroll_id: scrollId
  }) : client.search({
    body: {
  ...
  const close = async () => scrollId && (await client.clearScroll({
    scroll_id: scrollId
  }));
...
async function write(client, index, docs) {
  const {
    body
  } = await client.bulk({
  ...
  const err = _lodash.default.find(body.items, 'index.error.reason');
...
async function migrationsUpToDate(client, index, migrationVersion, kibanaVersion, retryCount = 10) {
  try {
    var _indexInfo$mappings$p;

    const indexInfo = await fetchInfo(client, index);

    if (!((_indexInfo$mappings$p = indexInfo.mappings.properties) !== null && _indexInfo$mappings$p !== void 0 && _indexInfo$mappings$p.migrationVersion)) {
      return false;
...
async function convertToAlias(client, info, alias, batchSize, script) {
  await client.indices.create({
    body: {
  ...
  await claimAlias(client, info.indexName, alias, [{
    remove_index: {
      index: alias
    }
  }]);
...
async function claimAlias(client, index, alias, aliasActions = []) {
  const {
    body,
    statusCode
  ...
  await client.indices.updateAliases({
    body: {
      actions: aliasActions.concat(removeActions).concat({
        add: {
          index,
          alias
        }
...
function assertIsSupportedIndex(indexInfo) {
  ...
  if (!isV7Index) {
    throw new Error(`Index ${indexInfo.indexName} belongs to a version of Kibana ` + `that cannot be automatically migrated. Reset it or use the X-Pack upgrade assistant.`);
  }
...
function assertResponseIncludeAllShards({
  _shards
}) {
  if (!_lodash.default.has(_shards, 'total') || !_lodash.default.has(_shards, 'successful')) {
    return;
  }
  ...
  if (failed > 0) {
    throw new Error(`Re-index failed :: ${failed} of ${_shards.total} shards failed. ` + `Check Elasticsearch cluster health for more information.`);
  }
}
/**
...
async function reindex(client, source, dest, batchSize, script) {
