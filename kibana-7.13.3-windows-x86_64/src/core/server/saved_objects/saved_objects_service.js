...
class SavedObjectsService {
  ...
  async start({
    ...
    const createRepository = (esClient, includedHiddenTypes = []) => {
      return _repository.SavedObjectsRepository.createRepository(migrator, this.typeRegistry, kibanaConfig.index, esClient, this.logger.get('repository'), includedHiddenTypes);
    };
