...
function applyConfigOverrides(rawConfig, opts, extraCliOptions) {
  ...
  const merge = _lodash.default.partial(_lodash.default.merge, rawConfig);
  ...
  set('plugins.scanDirs', _lodash.default.compact([].concat(get('plugins.scanDirs'), opts.pluginDir)));
...
function _default(program) {
  ...
  command.description('Run the kibana server').collectUnknownOptions().option('-e, --elasticsearch <uri1,uri2>', 'Elasticsearch instances').option('-c, --config <path>', 'Path to the config file, use multiple --config args to include multiple config files', configPathCollector, [(0, _utils.getConfigPath)()]).option('-p, --port <port>', 'The port to bind to', parseInt).option('-q, --quiet', 'Deprecated, set logging level in your configuration').option('-Q, --silent', 'Prevent all logging').option('--verbose', 'Turns on verbose logging').option('-H, --host <host>', 'The host to bind to').option('-l, --log-file <path>', 'Deprecated, set logging file destination in your configuration').option('--plugin-dir <path>', 'A path to scan for plugins, this can be specified multiple ' + 'times to specify multiple directories', pluginDirCollector, [(0, _utils.fromRoot)('plugins')]).option('--plugin-path <path>', 'A path to a plugin which should be included by the server, ' + 'this can be specified multiple times to specify multiple paths', pluginPathCollector, []).option('--plugins <path>', 'an alias for --plugin-dir', pluginDirCollector).option('--optimize', 'Deprecated, running the optimizer is no longer required');

  if (!(0, _utils.isKibanaDistributable)()) {
    command.option('--oss', 'Start Kibana without X-Pack').option('--run-examples', 'Adds plugin paths for all the Kibana example plugins and runs with no base path');
  }
  ...
  command.action(async function (opts) {
    if (opts.dev && opts.devConfig !== false) {
      ...
      } catch (err) {// ignore, kibana.dev.yml does not exist
      }
    }
